...
async function getAvailable(url, options){
    const response = await fetchGet(url, options);
...
document.body.addEventListener('keydown',
    event => {
        if(current_active != "done"){
            if(available.includes(event.code)){
                if(event.key == "Backspace"){
                    ...
                    all_type++;
                    typed_length++;
                    if(event.key == array_split[typed_length - 1]){
                        current_active = "green";
                        success_type++;
                    }else{
                        current_active = "red";
                    ...
                    if(keyboard_log.lastElementChild){
                        if(keyboard_log.lastElementChild.className == current_active){
                            if(is_space == 1){
                                keyboard_log.insertAdjacentHTML(
                                    'beforeend',
                                    '<span class="' + current_active + '">' + event.key + '</span>'
                                );
                            }else{
